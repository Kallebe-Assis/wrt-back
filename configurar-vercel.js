const fs = require('fs');

console.log('🚀 CONFIGURAÇÃO VERCELL - WRTmind Backend');
console.log('==========================================\n');

console.log('📋 PASSO A PASSO PARA CONFIGURAR NO VERCELL:\n');

console.log('1️⃣ Acesse o Dashboard do Vercel:');
console.log('   https://vercel.com/dashboard\n');

console.log('2️⃣ Selecione seu projeto: wrt-back\n');

console.log('3️⃣ Vá para: Settings > Environment Variables\n');

console.log('4️⃣ Adicione as seguintes variáveis (uma por uma):\n');

const envVars = {
  'FIREBASE_PROJECT_ID': 'wrtmind',
  'FIREBASE_PRIVATE_KEY': '-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n',
  'FIREBASE_CLIENT_EMAIL': 'firebase-adminsdk-fbsvc@wrtmind.iam.gserviceaccount.com',
  'JWT_SECRET': 'wrtmind-jwt-secret-2024-super-secure-key-change-this-in-production',
  'SESSION_SECRET': 'wrtmind-session-secret-2024-super-secure-key-change-this-in-production',
  'ALLOWED_ORIGINS': 'http://localhost:3000,https://wrtmind.vercel.app,https://wrt-frontend.vercel.app',
  'RATE_LIMIT_WINDOW_MS': '900000',
  'RATE_LIMIT_MAX_REQUESTS': '100',
  'LOG_LEVEL': 'info',
  'LOG_FILE': 'logs/app.log'
};

Object.entries(envVars).forEach(([key, value], index) => {
  console.log(`${index + 1}. ${key}:`);
  console.log(`   Valor: ${value}`);
  console.log('');
});

console.log('5️⃣ Clique em "Save" após adicionar cada variável\n');

console.log('6️⃣ Vá em: Deployments > Redeploy\n');

console.log('7️⃣ Aguarde o deploy completar (2-5 minutos)\n');

console.log('8️⃣ Teste a API:\n');
console.log('   URL: https://wrt-back.vercel.app/api/status');
console.log('   Deve retornar: {"firebase":"configured"}\n');

console.log('⚠️  IMPORTANTE:');
console.log('   - A FIREBASE_PRIVATE_KEY já está formatada corretamente');
console.log('   - Copie e cole exatamente como está acima');
console.log('   - Não adicione espaços extras');
console.log('   - Após adicionar todas, faça redeploy\n');

console.log('🎯 RESULTADO ESPERADO:');
console.log('   - Firebase: "configured" em vez de "not_configured"');
console.log('   - API funcionando corretamente');
console.log('   - Conexão com banco estabelecida\n');

console.log('📞 Se precisar de ajuda, verifique os logs do Vercel!'); 